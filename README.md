...
## Repository layout
- `client/` – Angular UI for browsing, creating, editing, and deleting realties. Uses Keycloak for auth and Angular Material for layout.
...
- `mongo/init-mongo.js` – seeds Mongo with credentials (`root`/`root`) and a sample realty document.
...
- **Backend:** `cd resource-server && gradle bootRun` (requires MongoDB and Keycloak running locally; credentials and URLs match `src/main/resources/application.yaml`).
- **Frontend:** `cd client && npm install && npm start` (Angular CLI 8). The app expects Keycloak at `http://localhost:8080`.

## Testing
- Backend: `gradle test` (use Gradle 5–6 to avoid plugin compatibility issues).
